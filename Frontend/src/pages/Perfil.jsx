...
import { buscarPerfil, atualizarPreferencias } from "../services/api.js";
...
export default function Perfil({ onSair }) {
  ...
  async function carregarPerfil() {
    try {
      ...
      if (data.usuario.preferencias) {
        setPrefs({
          not:    data.usuario.preferencias.notificacoesHorarios ?? true,
          tarifa: data.usuario.preferencias.alertasTarifa ?? false,
          news:   data.usuario.preferencias.newsletter ?? false,
        });
      }
    } catch (e) {
      console.error(e);
  ...
  async function togglePref(key) {
    ...
    setPrefs(p => ({ ...p, [key]: novoValor }));

    const mapBack = { not: "notificacoesHorarios", tarifa: "alertasTarifa", news: "newsletter" };
    try {
      await atualizarPreferencias({ [mapBack[key]]: novoValor });
    } catch (e) {
      console.error(e);
      setPrefs(p => ({ ...p, [key]: !novoValor }));
    }
  ...
  if (tela === "editar")     return <EditarDados  onVoltar={voltarERecarregar} perfil={perfil} />;
  if (tela === "senha")      return <AlterarSenha onVoltar={() => setTela(null)} />;
  if (tela === "endereco")   return <Endereco      onVoltar={voltarERecarregar} perfil={perfil} />;
  ...
  return (
    <div style={s.container}>
      ...
      <div style={s.avatarWrap}>
        <div style={s.avatar}>{getIniciais(nome)}</div>
        ...
        <div style={s.secao}>
          ...
          {[
            ...
            <div key={item.key} style={s.item}>
              <span style={s.itemLabel}>{item.label}</span>
              <button style={s.toggle(prefs[item.key])} onClick={() => togglePref(item.key)}>
